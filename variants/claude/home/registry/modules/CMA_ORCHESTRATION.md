...
model selection, effort escalation, or multi-agent governance.
...
- Use `planner-opus` for planner architecture or high-impact scope.
- Use `tdd-guide-opus` for tdd-guide safety-critical behavior or
  hardcoded-success traps.
- Keep code-reviewer and security-reviewer on `opus` / `medium`; they do
...
- Use `explorer-opus` for explorer complex incidents or conflicting evidence,
  and `docs-researcher-opus` for conflicting migration or security guidance.
- Keep default model/effort for routine work. Escalation is an explicit
  static role selection, not a persistent default change. All custom subagents
  use `medium`, and Opus variants explicitly pin `opus` / `medium`.

Load `${CLAUDE_CONFIG_DIR}/registry/ORCHESTRATION.md` only when detailed stage contracts or
