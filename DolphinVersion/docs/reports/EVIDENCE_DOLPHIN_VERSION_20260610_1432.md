...
- `MODEL_ID` is
  `cognitivecomputations_dolphin-mistral-24b-venice-edition`.
- Launcher path is `DolphinVersion/bin/dolphin`.
- Runtime home is `DolphinVersion/agent-home`.
...
- Forbidden legacy names were checked after the update and no file path matches
  remained under `DolphinVersion/`.
- Bash syntax validation passed for `DolphinVersion/bin/dolphin`.
